...
const chapters = {
  overview: { center: [-98, 39], zoom: 3 },
  california: { center: [-119.5, 36.7], zoom: 5 },
  east: { center: [-74, 40.7], zoom: 6 }
};
...
map.on('load', () => {
  ...
  map.addSource("overdose", { type: "geojson", data });
  ...
  window.addEventListener("scroll", () => {
    sections.forEach(section => {
      ...
      if (rect.top < window.innerHeight / 2 && rect.bottom > 0) {
        ...
        if (chapters[step]) map.flyTo(chapters[step]);
      }
    });
  });
});
